...
class TestLinkHistoryParentUpdate(SavePointsTestCase):
    def test_link_history_parent_update(self):
        ...
        manifest_data = {"parent_file": str(old_blend_path), "versions": []}
        with manifest_path.open("w", encoding="utf-8") as f:
            json.dump(manifest_data, f)

        print(
            f"Created source history at {source_history_dir} with parent_file: {old_blend_path}"
        )
        ...
        self.assertTrue(
            new_history_path.exists(), "New history directory does not exist"
        )
        ...
        if actual_parent != str(new_blend_path):
            ...
            if actual_parent != Path(new_blend_path).as_posix():
                self.fail(
                    f"FAILURE: parent_file was not updated. Expected {new_blend_path}, got {actual_parent}"
                )
