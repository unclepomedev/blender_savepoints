...
class TestBatchRenderLog(SavePointsTestCase):
    def test_log_import_logic(self):
        ...
        with self.subTest(step="1. Create Dummy Log File"):
            ...
            try:
                ...
                self.assertEqual(
                    text_block.name, f"Log_{version_id}", "Text block name mismatch"
                )
                ...
                self.assertIn(
                    dummy_content, full_text, "Log content missing from Text Block"
                )
                self.assertIn(
                    "# SavePoints Batch Render Log", full_text, "Header missing"
                )
    ...
    def test_subprocess_failure_capture(self):
        ...
        with self.subTest(step="1. Execute Failing Script"):
            ...
            failing_script = (
                "import sys; sys.stderr.write('CRITICAL WORKER FAILURE'); sys.exit(1)"
            )
            ...
            try:
                ...
                with open(log_path, "w", encoding="utf-8") as log_handle:
                    ...
                    process = subprocess.Popen(
                        cmd, stdout=log_handle, stderr=log_handle
                    )
                    exit_code = process.wait()

                # --- Verify Exit Code ---
                self.assertNotEqual(
                    exit_code, 0, "Process should have failed (exit code != 0)"
                )
                ...
                self.assertIn(
                    ...
                    content,
                    "Stderr output was not captured in the log file",
                )
