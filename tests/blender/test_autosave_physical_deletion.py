...
class TestAutosavePhysicalDeletion(SavePointsTestCase):
    ...
    def test_delete_version_by_id_physical(self):
        ...
        with self.subTest(step="2. Execute Deletion and Verify Mechanics"):
            # We mock send2trash to ensure it's NOT used.
            # We mock load_manifest and save_manifest to isolate logic.
            # We DO NOT mock rmtree because we want to verify the file is actually gone in the temp dir.
    ...
    def test_autosave_uses_physical_deletion(self):
        ...
        with self.subTest(step="1. Configure Autosave Settings"):
            ...
            # Simulate that enough time has passed (last autosave was at t=500, now is t=100000)
        ...
        with self.subTest(step="2. Trigger Timer and Verify Calls"):
            ...
            # Mock delete_version_by_id to spy on arguments
            # Mock create_snapshot to avoid heavy lifting
