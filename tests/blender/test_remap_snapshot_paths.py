...
def create_dummy_library(path):
    # Save current to restore later
    current = bpy.data.filepath
    
    # Save a dummy lib file
    bpy.ops.wm.save_as_mainfile(filepath=str(path))
    
    # Restore (or just don't worry as we will set filepath manually in tests)
...
def main():
    ...
    try:
        ...
        with open(img_path, 'wb') as f: f.write(b'fake png')
