...
class TestInBlenderE2E(SavePointsTestCase):
    def test_e2e_flow_scenario(self):
        ...
        with self.subTest(step="1. Commit Version"):
            ...
            self.assertTrue(
                (history_dir / "manifest.json").exists(), "Manifest not created"
            )

            v001_dir = history_dir / "v001"
            self.assertTrue(v001_dir.exists(), "Version v001 folder not created")
            self.assertTrue(
                (v001_dir / "snapshot.blend_snapshot").exists(),
        ...
        with self.subTest(step="2. Checkout Snapshot"):
            ...
            # Verify Content
            self.assertIn(
                "TestCube_v1", bpy.data.objects, "Snapshot missing original data"
            )
            self.assertNotIn(
                "TransientSphere",
                bpy.data.objects,
            ...
            self.assertIsNotNone(
                parent_path_detected, "Snapshot Mode not detected internally"
            )
            self.assertEqual(
                Path(parent_path_detected), self.blend_path, "Parent path mismatch"
            )
        ...
        with self.subTest(step="4. Restore to Parent"):
            ...
            self.assertIn(
                "RestoredCone",
                bpy.data.objects,
                "Restored file missing object added during snapshot",
            )
            self.assertIn(
                "TestCube_v1", bpy.data.objects, "Restored file missing original data"
            )
            ...
            self.assertTrue(
                len(backups) > 0, "Backup file was not created during restore"
            )
