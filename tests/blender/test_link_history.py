    ...
    def test_link_history_scenario(self):
        ...
        with self.subTest(step="1. Prepare Orphan History"):
            ...
            manifest_data = {
                ...
                "versions": [{"id": "v001", "note": "Old version to link"}],
            }
            ...
            self.assertTrue(
                manifest_path.exists(), "Setup failed: Manifest not created"
            )
        ...
        with self.subTest(step="2. Verify Pre-conditions"):
            ...
            self.assertFalse(
                target_history_dir.exists(),
                "History dir should not exist before linking",
            )
        ...
        with self.subTest(step="3. Execute Link History"):
            ...
            res = bpy.ops.savepoints.link_history(
                "EXEC_DEFAULT", filepath=str(orphan_dir), directory=str(orphan_dir)
            )
        ...
        with self.subTest(step="4. Verify Link Result"):
            ...
            self.assertTrue(
                target_history_dir.exists(),
                f"History dir was not created at {target_history_dir}",
            )

            # 2. Source (Orphan) directory must be gone (moved)
            self.assertFalse(
                orphan_dir.exists(),
                "Orphan dir was not removed/moved (Original folder should be gone)",
            )
            ...
            self.assertTrue(
                ...
                "Manifest file missing in the new history location",
            )
