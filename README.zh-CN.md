...
5.  **Auto Save (自动保存)**:
    ...
    * **Safety Mode (安全模式)**: 为防止崩溃和中断，当您处于交互模式（例如雕刻、权重绘制）或正在渲染时，自动保存将被 **跳过**。保存操作会被推迟，并在您退出这些模式后立即自动执行。如果自动保存挂起时间过长，HUD 上会出现警告。
6.  **Disk Management & Protection (磁盘管理与保护)**:
    * **Snapshot Compression**: 默认在 **插件首选项 (Add-on Preferences)** 中启用。保持开启以节省磁盘空间；如果您优先考虑最大保存速度而不是文件大小，请将其关闭。
...
12. **Background GLB Export (后台 GLB 导出)**:
    ...
    - **导出**:
        ...
        - **Filename (文件名)**: 可选设置自定义文件名。
            - 默认: 使用原始 `.blend` 文件名。
            - 占位符: 使用 `{version}` 包含版本 ID (例如 `MyProp_{version}`)。
            - *注意*: `.glb` 扩展名会自动添加。如果您输入 `A.glb`，结果将是 `A.glb.glb`。
        - 点击 **Export GLB**。
    - 导出在后台运行，您可以继续工作。
