...
- **Retrieve Objects**: 過去のファイル全体を開くことなく、特定のバージョンからオブジェクトだけを現在のシーンにインポート（アペンド）できます。
- **Ghost Reference**: 過去のバージョンをワイヤーフレームとして現在のビューポートに重ねて表示し、変更箇所を視覚的に比較できます。
- **Object History**: オブジェクトを右クリックするだけで、そのオブジェクトの変更履歴（メッシュ編集や移動）を一覧表示します。**Show All Versions** 機能を使えば、スカルプトなどの微細な変更も含めたすべての過去の状態にアクセスし、**Ghost** でプレビュー可能です。
- **Batch Timelapse**: 複数のバージョンを一括レンダリングし、制作過程のタイムラプス動画を自動生成します。バージョン名の焼き込み機能付き。
- **Context-Aware Rendering**: **現在**のカメラ設定（位置・レンズ・シフト）、ワールド環境、カラーマネジメント（AgX/Filmic）、レンダリング設定を**過去**のスナップショットに適用します。
  <br>*(※コンポジットノードの設定はスナップショット当時のまま維持されます。)*
- **Snapshot Compression**: スナップショットファイルを圧縮するオプションを追加しました。保存時間はわずかに増えますが、ファイルサイズを大幅に削減できます。（デフォルト：**有効**）
