...
- **Batch Timelapse**: **(New!)** 複数のバージョンを一括レンダリングし、制作過程のタイムラプス動画を自動生成します。
...
10. **Batch Rendering & Timelapse**:
    ...
    - **レンダリング開始**: **Batch Render Selected** をクリックします。
      ...
      - **ドライラン (Dry Run)**: 通常クリックでダイアログを開き、**"Dry Run"** にチェックを入れると、低解像度（25%）・1サンプルの高速プレビューを実行します。
      - **即時本番レンダリング**: `Shift` を押しながらボタンをクリックすると、**ダイアログをスキップ**して即座に本番レンダリングを開始します。
