...
- **Object History**: **(New!)** オブジェクトを右クリックするだけで、そのオブジェクトの変更履歴（メッシュ編集や移動）を一覧表示します。過去の状態を**Ghost**として即座にプレビューすることも可能です。
...
### オブジェクト履歴に関する制限事項
Object History 機能は、完全なジオメトリ解析ではなく軽量なメタデータ（頂点数、バウンディングボックス、トランスフォーム行列）を使用することで、スナップショットファイルをすべて読み込むことなく瞬時に結果を表示します。そのため、以下の点にご注意ください。
- **内部的な変形**: バウンディングボックス全体や頂点数が変化しないメッシュ変形（例：平面上での頂点スライド移動など）は検知されない場合があります。
