...
class VertexWeightMixModifier(Modifier):
    ...
    def persistent_uid(self) -> Annotated[int, "step=1"]:
        ...
        ...
    vertex_group_a: Annotated[str, "is_animatable=False"]
    """First vertex group name"""
    vertex_group_b: Annotated[str, "is_animatable=False"]
    ...
    default_weight_a: Annotated[float, "subtype='FACTOR'", "step=1.0", "precision=-1"]
    """Default weight a vertex will have if it is not in the first A vgroup"""
    default_weight_b: Annotated[float, "subtype='FACTOR'", "step=1.0", "precision=-1"]
    """Default weight a vertex will have if it is not in the second B vgroup"""
    mix_mode: Literal['SET', 'ADD', 'SUB', 'MUL', 'DIV', 'DIF', 'AVG', 'MIN', 'MAX']
    """How weights from vgroup B affect weights of vgroup A"""
    mix_set: Literal['ALL', 'A', 'B', 'OR', 'AND']
