...
class VertexWeightEditModifier(Modifier):
    ...
    default_weight: Annotated[float, "subtype='FACTOR'", "step=1.0", "precision=-1"]
    """Default weight a vertex will have if it is not in the vgroup"""
    add_threshold: Annotated[float, "step=1.0", "precision=-1"]
    """Lower (inclusive) bound for a vertex's weight to be added to the vgroup"""
    remove_threshold: Annotated[float, "step=1.0", "precision=-1"]
