...
class SAVEPOINTS_UL_version_list(bpy.types.UIList):
    def draw_item(
        self, context, layout, _data, item, _icon, _active_data, _active_propname
    ):
    ...
    def filter_items(self, context, data, propname):
        ...
        if self.filter_name or filter_tag != "ALL":
            ...
            for item in items:
                ...
                if filter_text:
                    if not (
                        filter_text in item.version_id.lower()
                        or filter_text in item.note.lower()
                    ):
                        match_text = False
    ...
    if 0 <= settings.active_version_index < len(settings.versions):
        ...
        if settings.show_preview:
            ...
            if pcoll and item.version_id in pcoll:
                ...
                row.template_icon(
                    icon_value=pcoll[item.version_id].icon_id, scale=dynamic_scale
                )
            else:
                row = box.row()
                row.alignment = "CENTER"
