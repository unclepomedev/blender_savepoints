...
def setup_world(scene, settings):
    ...
    if world_name:
        if (
            world_name not in bpy.data.worlds
            and main_blend_path
            and os.path.exists(main_blend_path)
        ):
            try:
                with bpy.data.libraries.load(main_blend_path, link=False) as (
                    data_from,
                    data_to,
                ):
...
def setup_view_layer(scene, settings):
    ...
    if current_layer.name != target_layer_name:
        ...
        print(
            f"Worker: Renaming active ViewLayer '{current_layer.name}' -> '{target_layer_name}'"
        )
